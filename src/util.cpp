...
#else
    const char* pszModule = "cambtcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CambtCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CambtCoin
    // Mac: ~/Library/Application Support/CambtCoin
    // Unix: ~/.cambtcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CambtCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cambt.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cambtd.pid"));
