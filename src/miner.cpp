...
{
    LogPrintf("CambtCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cambtcoin-miner");
	...
	CBlock *pblock = pblocktemplate.get();
        ...
        LogPrintf("Running CambtCoinMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("CambtCoinMiner terminated\n");
        throw;
    }
}
